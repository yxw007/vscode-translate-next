...
import { getDefaultEngine, getParentLanguage } from "../common";
...
export async function getMarkdownTextValue(markStr: string) {
    ...
    async function translate(text: string) {
        if (!translatedTask) {
            const targetLanguage = getParentLanguage();
            const engine = getDefaultEngine() as Engines;
            translatedTask = translator.translate(textArr.join("\n"), { to: targetLanguage as ToLanguage<typeof engine>, engine}) as Promise<string>;
        }
