...
async function doLoadOnigasm(): Promise<IOnigLib> {
    const bytes = await fs.promises.readFile(path.join(__dirname, "./onigasm.wasm"));
...
export class TextMateService implements ITextMateService {
    ...
    private async _createGrammar(modeId: string): Promise<ICreateGrammarResult | null> {
        ...
        if (!languageRegistration) {
            ...
            //Modified to return null instead of throw, the translation executes normally
            logger.warn("No TM Grammar registered for this language.");
            return null;
        }
